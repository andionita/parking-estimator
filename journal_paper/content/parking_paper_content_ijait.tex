...
For an overview of smart parking projects, including development cost and the number of parking spaces covered, see Figure \Cref{fig:smart_parking_projects}.
...
As in Section \Cref{sec:relwork_smart_parking}, all relevant work found on the topic are listed in Tables \Cref{tab:related_work2} and \Cref{tab:related_work3}. The key data shown emphasizes the prediction character of the papers by highlighting their general approach, features, methods, and evaluation.
...
The merge distance can be intuitively understood as the radius around a public amenity. It is defined to represent the parking area that is relevant for a particular public amenity, or, more straightforward, the walking distance from the parked car to, e.g., the restaurant, the office, the bank, etc. Concrete instances of the merging distance can be found in Section \Cref{experimental_setup:merging_parking_city_data}.
...
Compare Figure \Cref{fig:cluster_vector} for a general representation.
...
Compare Figure \Cref{fig:gaussian} for a visualization of the summing process.
...
We choose to explore this methodology, following the solid results machine learning models have delivered for the various smart parking settings investigated in Section \Cref{sec:relwork}. A machine learning model $\mathcal{M}$ will be trained for every cluster \textit{with} parking data.
...
The SF\textit{park} data are visualized in Figure \Cref{fig:before_clustering} using a built Leaflet application.
...
As established in Section \Cref{concept:clustering}, we apply K-Means to cluster the city areas.
...
The clusters \textit{with} parking data will turn dark red, while the clusters \textit{without} parking data will appear in dark blue (cf. Figure \Cref{fig:highlighted_collage}). 
...
As defined in Section \Cref{realization:vectors}, the categories are based on the visiting duration mean.
...
\end{romanlist}
The assigned partitions for every amenity are shown in a separator column in \Cref{tab:amenities_google_places}. 

The computation of \textit{cluster Gaussians} relies on both the mean and standard deviation of the amenity visiting duration, as defined in Section \Cref{realization:gaussians}.

\subsection{Amenity area as similarity basis}
\label{experimental_setup:amenity_area}
Together with the similarity functions in Section \Cref{experimental_setup:urban_measure} we considered \textit{visiting duration} as the basis for creating the urban profile. However, there are other measures that reflect the parking demand towards an amenity, one of them being the area of the particular amenity. OpenStreeMap provides a polygon layer for a certain geographic bounding box, which contains information across all the surfaces in that region (recall Section \Cref{sec:citydata}). See Figure \Cref{fig:amenity_polygons} for a visualization of the polygons and their areas in OSM. In extracting this information we had two options. 
...
In Figure \Cref{fig:cwith} a screenshot of the web application showing a sample of the results is illustrated. Figure \Cref{fig:cwith_table} displays the presented table in more detail.
...
\begin{figure}[!ht]
	...
	\caption{The pop-up table for the Leaflet application view of Figure \Cref{fig:cwith}.}
...
An example of the estimation is visualized Figure \Cref{fig:cwout_table}.
