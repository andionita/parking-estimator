...
Instead of aggregating data points for the machine learning models over the timestamp, we build the cluster models using the entire occupancy data. 
...
%			H\"ossinger \cite{hoessinger} & 2014 & parking prediction & \begin{tabular}{@{}c@{}}parking tickets\\ mantiual counts\\ traffic flow\end{tabular}& on-street & Vienna \\ \hline
...
The practical take-away idea when working with clusters is that the location information, albeit not precise down to the street block, offers a clear indication on the area where a driver should search for parking.  
...
With one single exception, when the simple total models achieve a better prediction performance, the addition of the two features did not help with finding better parking occupancy values for the test clusters, at least using the gradient boost model we used in our experiments.
...
\begin{romanlist}
	...
	Our approach is dependent on the fact that the computed K-Means clusters are of approximately the same size. Ideally, this would mean that the number of amenities of a certain visiting duration is virtually equal between clusters. But since for both cosine similarity and emd it is not the absolute number of amenities for a certain visiting duration value that matters, instead the relative number between certain visiting duration values, it may be that clusters of clearly opposite sizes are very similar. All the more is the reason to experiment with other clustering methods, such as DBSCAN and OPTICS. Both focus on detecting clusters based on density of neighborhoods, which may translate in practice into separating regions in the city that are more sparse, e.g., large office areas, from regions that are very dense, e.g., residential or old town areas.
