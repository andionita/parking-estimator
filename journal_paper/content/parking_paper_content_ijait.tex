...
To construct a cluster Gaussian, we first collect the average visiting duration and standard deviation of the individual amenities.
...
The \textit{earth mover's distance (emd)} is a measure used in statistics that roughly expresses the difference between position and magnitude of two curves.
...
More rigorously, the earth mover's distance is better known in mathematics under the name \textit{Wasserstein Metric}.
...
First off, a number of so-called \textit{bins} is determined.
Each bin represents a unit on the $X$ axis, the same on which the visiting duration is expressed.
We will take a number of bins equal to the maximum amenity mean and add $3\times$ the largest standard deviation to it, as it is known that within $3\times$ standard deviation on both sides of the mean over 99\% of the Gaussian sum is covered.
...
\Cref{evaluation:similarity_vs_estimation_sec} is the heart of the evaluation. 
...
Then, in \cref{evaluation:estimations_cwout}, we take a look at the results of applying the models to clusters \textit{without parking data} and showcase the web application. \Cref{evaluation:entire_datapoints} looks at the model test errors and correlation results by skipping the aggregating step, i.e. instead of averaging the datapoints over timestamp per cluster, we build the cluster models using the entire occupancy data directly. 
...
In \cref{tab:correlation_values} the resulting correlations of testing errors with cosine- and emd similarity values are listed for models of 8 and 16 cluster configurations. Regarding the number of clusters, the values for 8-cluster models are clearly superior to the 16-cluster models. 
	\todo[inline]{MC I disagree with this conclusion.
	The best result for all datapoints is for cluster size 16 in table 9...
}
When comparing by target datapoints type, the models applied on aggregated datapoints have a slight edge, with the cosine- and rank cosine correlation values being on average closer to -1 and emd- and rank emd correlation values being closer to 1. However, for 16 clusters the correlations tendency is barely noticeable.
\todo[inline]{MC There are two experiments: all and aggregate. Each of the experiments have a best result.
Still, both are relevant as giving a driver a recommendation to search in a given area, instead of at a very specific place is still useful.
Perhaps that point should be added to section 6 or even before.
}
\begin{table}[!ht]
	%\centering
	%\small
...
Together with the similarity functions we considered \textit{timespent} as the basis for creating the urban profile (recall \ref{experimental_setup:urban_measure}). 
\todo[inline]{MC I think this was too repetitive. I also merged these two subsections.  }
...
\todo[inline]{MC I added the following extra info. Is this the model used in these experiments? The point in adding this is that it might just be that the gradient boosting is the best model for this data.} 
using the gradient boost model we used in our experiments.
...
\begin{romanlist}
	...
	Our approach is dependent on the fact that the computed k-Means clusters are of approximately same size. Such clusters are likely to contain a similar number of amenities and similarity values can be accurately computed based on them, e.g. cosine similarity is dependent on the absolute number of amenities in each category
	\todo[inline]{MC No, it is not.}
	...
	\todo[inline]{MC Why do you call this semi-supervised?}
	...
	\todo[inline]{MC I came up with the following inspired by the previous one. Is it the same?}
...
\todo[inline]{MC The next sentence is too complicated to read. I am not even 100\% sure what it means.}
...
\todo[inline]{MC Perhaps add: However, making these areas too small results in models which do not generalize as well.}
...
%\section{Note}
%Some inconsistencies in the evaluation results for, e.g. testing errors built with the same models for the same data aggregation and other attributes, might be observed. 
%Please note that the clustering associated with a experiment run is different every time (due to its random seed) and therefore this may result in e.g. testing errors that may slightly differ for the same configuration in various stages of the evaluation. 
%We cross-checked the results and intended to keep everything consistent but since the exact values are not 100\% reproducible we decided to leave some parts as they are while pointing out their tendency and meaning in each case.
