...
The types of the public amenities collected from the POIs are listed in \ref{tab:amenities_list}.

\begin{table}[!ht]
	%\scriptsize
	\tbl{List of all OSM amenity types found in the SF\textit{park} blocks.}
...
The calculation of \textit{cluster Gaussians} relies on both the mean and standard deviation of the amenity visiting duration, as defined in \ref{realization:gaussians}.
...
Secondly, in \ref{extensions:amenity_area} we use another amenity value on which of the similarity functions are applied (recall \ref{extensions:amenity_area}). Instead of \textit{amenity time spent}, \textit{amenity area} will extracted and leveraged. 
...
\begin{table}[!ht]
	%\centering
	%\small
	\tbl{Train and test error for the ML models build with all datapoints}
	%\setlength\extrarowheight{5pt}
	%\resizebox{\textwidth}{!}{
	{\begin{tabular}{ | c | c | c | c | c | c | }
		\hline
...
\begin{table}[!ht]
	...
	%\setlength\extrarowheight{5pt}
	%\resizebox{\textwidth}{!}{
	\tbl{Resulting correlation values for ML models built using all datapoints alongside correlations of models based on aggregate datapoints for 8, 16 and 32 clusters}
	{\begin{tabular}{ | c | c | c | c | c | c | }
		\hline
...
\begin{table}[!ht]
	%\footnotesize
	\tbl{Amenity area values gathered from OMS polygon layer for the SF\textit{park} region}
	{\begin{tabular}{ | l | c | c | c || l | c | c | c |}
		\hline		
		\textbf{amenity name} & \textbf{mean} & \textbf{stdev} & \textbf{cat} & \textbf{amenity name} & \textbf{mean} & \textbf{stdev} & \textbf{cat} \\ \hline
...
\begin{table}[!ht]
		...
		%\setlength\extrarowheight{5pt}
		%\resizebox{\textwidth}{!}{
		\tbl{The correlation results based on the amenity area values.}
...
\begin{table}[!ht]
	%\footnotesize
	\tbl{Models extended with cosine and emd features compared to previous models with regard to training- and testing errors}
	{\begin{tabular}{ | c | c | c | c | }
		\hline		
